	...
	_insertHint(false)
{
	...
	SQLSMALLINT t;
	SQLRETURN r = Poco::SQL::ODBC::SQLGetInfo(_rConnection, SQL_DRIVER_NAME, NULL, 0, &t);
	if (!Utility::isError(r) && t > 0)
	{
		std::string serverString;
		serverString.resize(static_cast<std::size_t>(t) + 2);
		r = Poco::SQL::ODBC::SQLGetInfo(_rConnection, SQL_DRIVER_NAME, &serverString[0], SQLSMALLINT((serverString.length() - 1) * sizeof(serverString[0])), &t);
	}
}
...
void ODBCStatementImpl::doPrepare()
{
	if (!_prepared && session().getFeature("autoExtract") && hasData())
	{
		...
		if (it != itEnd && (*it)->isBulk())
		{
			...
			checkError(Poco::SQL::ODBC::SQLSetStmtAttr(_stmt, SQL_ATTR_ROW_ARRAY_SIZE, (SQLPOINTER) limit, 0),
					"SQLSetStmtAttr(SQL_ATTR_ROW_ARRAY_SIZE)");
		}

		AbstractPreparation::Ptr pAP = 0;
		Poco::SQL::AbstractPreparator::Ptr pP = _preparations[curDataSet];
