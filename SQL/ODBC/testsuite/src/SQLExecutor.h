...
#define poco_data_using_statements using Poco::SQL::Keywords::now; \
		using Poco::SQL::Keywords::into; \
		using Poco::SQL::Keywords::use; \
		using Poco::SQL::Keywords::bulk; \
		using Poco::SQL::Keywords::limit; \
		using Poco::SQL::CLOB; \
...
public:
	...
	SQLExecutor(const std::string& name, Poco::SQL::Session* _pSession, const std::string& connInitSql = std::string(), const std::string& schemaName = std::string());
	~SQLExecutor();
...
inline Poco::SQL::Session& SQLExecutor::session()
{
