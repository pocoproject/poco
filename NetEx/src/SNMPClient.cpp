...
// SNMPClient.cpp
//
// $Id: //poco/1.4/NetEx/src/SNMPClient.cpp#1 $
//
// Library: NetEx
// Package: SNMP
// Module:  SNMPClient
...
void SNMPClient::send(SocketAddress& address, SNMPTypes::SNMPMessage::Ptr message)
{
	...
	try
	{
		...
		while (true)
		{
			Poco::ASN::ASN1::Ptr resp = snmpClient.query(address, message->encode());
	...
	const std::string &community, SNMPVersion::Version version)
{
	...
	message->pdu().varBindList().add(new Poco::Net::SNMPTypes::VarBind(oid, new Poco::ASN::ASN1Types::Null()));
...
void SNMPClient::set(SocketAddress& address, const std::string &oid, ASN::ASN1::Ptr value, int requestId, 
	const std::string &community, SNMPVersion::Version version)
{
	SNMPTypes::SNMPMessage::Ptr message(new SNMPTypes::SNMPMessage);
	message->setVersion(version);
...
void SNMPClient::set(const std::string& address, const std::string &oid, ASN::ASN1::Ptr value, int requestId, 
	const std::string &community, SNMPVersion::Version version)
{
	SocketAddress addr(address, 161);
