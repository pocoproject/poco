...
// SNMPClientRaw.cpp
//
// $Id: //poco/1.4/NetEx/src/SNMPClientRaw.cpp#1 $
//
// Library: NetEx
// Package: SNMP
...
ASN::ASN1::Ptr SNMPClientRaw::query(const std::string& address, ASN::ASN1::Ptr query)
{
...
ASN::ASN1::Ptr SNMPClientRaw::query(SocketAddress& address, ASN::ASN1::Ptr query)
{
	std::stringstream data;
	Poco::SharedPtr<ASN::ASN1Factory> factory(new SNMPClientRawFactory);
	ASN::ASN1Codec codec(factory);
	...
	ASN::ASN1::Ptr response = codec.decode(srec);

	return response;
}


void SNMPClientRaw::send(const std::string& address, ASN::ASN1::Ptr query)
{
	SocketAddress addr(address, 161);
	send(addr, query);
}
...
void SNMPClientRaw::send(SocketAddress& address, ASN::ASN1::Ptr query)
{
	SNMPRawEventArgs eventArgs(address);
	try
	{
		ASN::ASN1::Ptr response = this->query(address, query);
