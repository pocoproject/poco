...
{
public:
	typedef AutoPtr<Document> Ptr;

	explicit Document(AutoPtr<NamePool> pNamePool = 0);
	...
	Document(AutoPtr<DocumentType> pDocumentType, AutoPtr<NamePool> pNamePool = 0);
		/// Creates a new document. If pNamePool == 0, the document
		/// creates its own name pool, otherwise it uses the given name pool.
		/// Sharing a name pool makes sense for documents containing instances
		/// of the same schema, thus reducing memory usage.

	Document(AutoPtr<DocumentType> pDocumentType, unsigned long namePoolSize);
		/// Creates a new document using a name pool with the given size, which
		/// should be a prime number (e.g., 251, 509, 1021, 4093).
	...
	AutoPtr<DocumentType> doctype() const;
	...
	AutoPtr<DocumentFragment> createDocumentFragment() const;
		/// Creates an empty DocumentFragment object.

	AutoPtr<Text> createTextNode(const XMLString& data) const;
		/// Creates a text node given the specified string.

	AutoPtr<Comment> createComment(const XMLString& data) const;
		/// Creates a comment node given the specified string.

	AutoPtr<CDATASection> createCDATASection(const XMLString& data) const;
		/// Creates a CDATASection node whose value is the specified string.

	AutoPtr<ProcessingInstruction> createProcessingInstruction(const XMLString& target, const XMLString& data) const;
		/// Creates a ProcessingInstruction node given the specified target and data strings.

	AutoPtr<Attr> createAttribute(const XMLString& name) const;	
	...
	AutoPtr<EntityReference> createEntityReference(const XMLString& name) const;
	...
	AutoPtr<NodeList> getElementsByTagName(const XMLString& name) const;
	...
	AutoPtr<Attr> createAttributeNS(const XMLString& namespaceURI, const XMLString& qualifiedName) const;
		/// Creates an attribute of the given qualified name and namespace URI.

	AutoPtr<NodeList> getElementsByTagNameNS(const XMLString& namespaceURI, const XMLString& localName) const;
	...
	// Extensions
	AutoPtr<Entity> createEntity(const XMLString& name, const XMLString& publicId, const XMLString& systemId, const XMLString& notationName) const;
	...
	AutoPtr<Notation> createNotation(const XMLString& name, const XMLString& publicId, const XMLString& systemId) const;
...
protected:
	...
	AutoPtr<Node> copyNode(bool deep, Ptr pOwnerDocument) const;

	AutoPtr<DocumentType> getDoctype();

	void setDoctype(AutoPtr<DocumentType> pDoctype);

private:
	AutoPtr<DocumentType> _pDocumentType;
