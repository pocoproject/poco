...
{
public:
	typedef RefPtr<ParserEngine> Ptr;
	...
	void setDeclHandler(RefPtr<DeclHandler> pDeclHandler);
		/// Allow an application to register a DTD declarations event handler.

	RefPtr<DeclHandler> getDeclHandler() const;
	...
	void setLexicalHandler(RefPtr<LexicalHandler> pLexicalHandler);
		/// Allow an application to register a lexical event handler.

	RefPtr<LexicalHandler> getLexicalHandler() const;
	...
	void parse(RefPtr<InputSource> pInputSource);
...
protected:
	...
	void parseExternal(XML_Parser extParser, RefPtr<InputSource> pInputSource);
		/// Parse an XML document from the given InputSource.
	...
	void pushContext(XML_Parser parser, RefPtr<InputSource> pInputSource);
...
private:
	...
	RefPtr<DTDHandler>     _pDTDHandler;
	RefPtr<DeclHandler>    _pDeclHandler;
	RefPtr<ContentHandler> _pContentHandler;
	RefPtr<LexicalHandler> _pLexicalHandler;
	RefPtr<ErrorHandler>   _pErrorHandler;
