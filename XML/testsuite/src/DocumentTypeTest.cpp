...
void DocumentTypeTest::testDocumentType()
{
	...
	assert (!pDoctype->ownerDocument().isNull());
	...
	assert (pNotations->length() == 0);
	assert (pEntities->item(0).cast<Entity>() == pEntity1);
	assert (pEntities->getNamedItem("entity1").cast<Entity>() == pEntity1);
	...
	assert (pNotations->length() == 0);
	assert (pEntities->item(0).cast<Entity>() == pEntity1);
	assert (pEntities->item(1).cast<Entity>() == pEntity2);
	assert (pEntities->getNamedItem("entity1").cast<Entity>() == pEntity1);
	assert (pEntities->getNamedItem("entity2").cast<Entity>() == pEntity2);
	...
	assert (pEntities->item(0).cast<Entity>() == pEntity1);
	assert (pEntities->item(1).cast<Entity>() == pEntity2);
	assert (pNotations->item(0).cast<Notation>() == pNotation);
	assert (pEntities->getNamedItem("entity1").cast<Entity>() == pEntity1);
	assert (pEntities->getNamedItem("entity2").cast<Entity>() == pEntity2);
	assert (pNotations->getNamedItem("notation").cast<Notation>() == pNotation);
}
