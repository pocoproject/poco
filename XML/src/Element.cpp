...
Attr::Ptr Element::getAttributeNode(const XMLString& name) const
{
	...
	while (pAttr && pAttr->_name.qname() != name) pAttr = pAttr->_pNext.unsafeCast<Attr>();
	return pAttr;
}
...
{
	return new ElementsByTagNameList(AutoPtr<Element>(const_cast<Element*>(this), true), name);
}


NodeList::Ptr Element::getElementsByTagNameNS(const XMLString& namespaceURI, const XMLString& localName) const
{
	return new ElementsByTagNameListNS(AutoPtr<Element>(const_cast<Element*>(this), true), namespaceURI, localName);
}
...
{
	return new AttrMap(AutoPtr<Element>(const_cast<Element*>(this), true));
}
...
Element::Ptr Element::getElementById(const XMLString& elementId, const XMLString& idAttribute) const
{
	if (getAttribute(idAttribute) == elementId)
		return AutoPtr<Element>(const_cast<Element*>(this), true);
