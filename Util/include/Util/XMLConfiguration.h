...
// XMLConfiguration.h
//
// $Id: //poco/1.1.0/Util/include/Util/XMLConfiguration.h#2 $
//
// Library: Util
...
public:
	...
	XMLConfiguration(XML::InputSource* pInputSource);
	...
	XMLConfiguration(const XML::Document* pDocument);
		/// Creates the XMLConfiguration using the given XML document.
		
	XMLConfiguration(const XML::Node* pNode);
		/// Creates the XMLConfiguration using the given XML node.

	void load(XML::InputSource* pInputSource);
	...
	void load(const XML::Document* pDocument);
		/// Loads the XML document containing the configuration data
...
private:
	const XML::Node* findNode(const std::string& key) const;
	static const XML::Node* findNode(std::string::const_iterator& it, const std::string::const_iterator& end, const XML::Node* pNode);
	static const XML::Node* findElement(const std::string& name, const XML::Node* pNode);
	static const XML::Node* findElement(int index, const XML::Node* pNode);
	static const XML::Node* findAttribute(const std::string& name, const XML::Node* pNode);

	XML::AutoPtr<XML::Node>     _pRoot;
	XML::AutoPtr<XML::Document> _pDocument;
};
