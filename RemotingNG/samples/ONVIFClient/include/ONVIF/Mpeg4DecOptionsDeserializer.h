...
#ifndef TypeDeserializer_ONVIF_Mpeg4DecOptions_INCLUDED
#define TypeDeserializer_ONVIF_Mpeg4DecOptions_INCLUDED
...
template <>
class TypeDeserializer<ONVIF::Mpeg4DecOptions>
{
public:
	static bool deserialize(const std::string& name, bool isMandatory, Deserializer& deser, ONVIF::Mpeg4DecOptions& value)
	{
	...
	static void deserializeImpl(Deserializer& deser, ONVIF::Mpeg4DecOptions& value)
	{
		using namespace std::string_literals;
		
		static const std::string REMOTING__NAMES[] = {"ResolutionsAvailable"s,"SupportedMpeg4Profiles"s,"SupportedInputBitrate"s,"SupportedFrameRate"s};
		bool ret = false;
		...
		std::vector<std::string> gen_supportedMpeg4Profiles;
		ret = TypeDeserializer<std::vector<std::string>>::deserialize(REMOTING__NAMES[1], true, deser, gen_supportedMpeg4Profiles);
		if (ret) value.setSupportedMpeg4Profiles(gen_supportedMpeg4Profiles);
...
#endif // TypeDeserializer_ONVIF_Mpeg4DecOptions_INCLUDED
