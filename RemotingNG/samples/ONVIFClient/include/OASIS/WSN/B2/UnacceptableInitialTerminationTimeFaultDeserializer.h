//
// UnacceptableInitialTerminationTimeFaultDeserializer.h
//
// Package: Generated
...
#ifndef TypeDeserializer_OASIS_WSN_B2_UnacceptableInitialTerminationTimeFault_INCLUDED
#define TypeDeserializer_OASIS_WSN_B2_UnacceptableInitialTerminationTimeFault_INCLUDED
...
template <>
class TypeDeserializer<OASIS::WSN::B2::UnacceptableInitialTerminationTimeFault>
{
public:
	static bool deserialize(const std::string& name, bool isMandatory, Deserializer& deser, OASIS::WSN::B2::UnacceptableInitialTerminationTimeFault& value)
	{
	...
	static void deserializeImpl(Deserializer& deser, OASIS::WSN::B2::UnacceptableInitialTerminationTimeFault& value)
	{
		using namespace std::string_literals;
		...
		static const std::string REMOTING__NAMES[] = {"MinimumTime"s,"MaximumTime"s};
		bool ret = false;
		deser.pushProperty(SerializerBase::PROP_XSDTYPE, "dateTime"s);
		Poco::DateTime gen_minimumTime;
		ret = TypeDeserializer<Poco::DateTime>::deserialize(REMOTING__NAMES[0], true, deser, gen_minimumTime);
		if (ret) value.setMinimumTime(gen_minimumTime);
		deser.popProperty(SerializerBase::PROP_XSDTYPE);
		deser.pushProperty(SerializerBase::PROP_XSDTYPE, "dateTime"s);
		Poco::Optional<Poco::DateTime> gen_maximumTime;
		ret = TypeDeserializer<Poco::Optional<Poco::DateTime>>::deserialize(REMOTING__NAMES[1], false, deser, gen_maximumTime);
		if (ret) value.setMaximumTime(gen_maximumTime);
		deser.popProperty(SerializerBase::PROP_XSDTYPE);
	}
...
#endif // TypeDeserializer_OASIS_WSN_B2_UnacceptableInitialTerminationTimeFault_INCLUDED
