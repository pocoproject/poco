...
public:
	...
	static void serializeImpl(const Pizzeria::Order& value, Serializer& ser)
	{
		...
		static const std::string REMOTING__NAMES[] = {"orderNumber"s,"pizza"s,"deliveryAddress"s,"deliveryTime"s,""s};
		TypeSerializer<Poco::Optional<int>>::serialize(REMOTING__NAMES[0], value.orderNumber, ser);
		TypeSerializer<Pizzeria::Pizza>::serialize(REMOTING__NAMES[1], value.pizza, ser);
		TypeSerializer<Pizzeria::DeliveryAddress>::serialize(REMOTING__NAMES[2], value.deliveryAddress, ser);
		TypeSerializer<Poco::Nullable<Poco::DateTime>>::serialize(REMOTING__NAMES[3], value.deliveryTime, ser);
	}
