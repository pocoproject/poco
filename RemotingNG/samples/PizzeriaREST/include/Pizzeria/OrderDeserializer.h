...
public:
	...
	static void deserializeImpl(Deserializer& deser, Pizzeria::Order& value)
	{
		...
		static const std::string REMOTING__NAMES[] = {"orderNumber"s,"pizza"s,"deliveryAddress"s,"deliveryTime"s};
		TypeDeserializer<Poco::Optional<int>>::deserialize(REMOTING__NAMES[0], true, deser, value.orderNumber);
		TypeDeserializer<Pizzeria::Pizza>::deserialize(REMOTING__NAMES[1], true, deser, value.pizza);
		TypeDeserializer<Pizzeria::DeliveryAddress>::deserialize(REMOTING__NAMES[2], true, deser, value.deliveryAddress);
		TypeDeserializer<Poco::Nullable<Poco::DateTime>>::deserialize(REMOTING__NAMES[3], true, deser, value.deliveryTime);
	}
