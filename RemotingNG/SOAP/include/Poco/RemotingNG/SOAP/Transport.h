...
public:
		...
		/// Authentication mode.
	{
		...
		AUTH_WSSE_TEXT   = 4, /// WS-Security Username Token Profile with plaintext password
		AUTH_WSSE_DIGEST = 5  /// WS-Security Username Token Profile with password digest
	};

	using RequestHeaderInterceptor = Serializer::HeaderInterceptor;
	using ResponseHeaderInterceptor = Deserializer::HeaderInterceptor;
	...
	void addRequestHeaderInterceptor(RequestHeaderInterceptor::Ptr pInterceptor);
		/// Adds a header interceptor to the request serializer.

	void removeRequestHeaderInterceptor(RequestHeaderInterceptor::Ptr pInterceptor);
		/// Removes a header interceptor from the request serializer.

	void addResponseHeaderInterceptor(ResponseHeaderInterceptor::Ptr pInterceptor);
		/// Adds a header interceptor to the response deserializer.
...
private:
	...
	Poco::SharedPtr<WSSEInterceptor> _pWSSEInterceptor;
