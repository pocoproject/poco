...
// ExpireStrategy.h
//
// $Id: //poco/1.1.0/Foundation/include/Foundation/ExpireStrategy.h#2 $
//
// Library: Foundation
// Package: Cache
...
public:
	...
	{
		poco_assert_dbg(_keys.find(args.key()) == _keys.end());
		Timestamp now;
		IndexIterator it = _keyIndex.insert(std::make_pair(now, args.key()));
		_keys.insert(std::make_pair(args.key(), it));
	}
...
protected:
	Foundation::Timestamp::TimeDiff _expireTime;
