...
public:
	...
	[[nodiscard]] virtual std::size_t digestLength() const = 0;
	...
	[[nodiscard]] virtual const Digest& digest() = 0;
	...
	[[nodiscard]] static std::string digestToHex(const Digest& bytes, std::size_t length = 0);
	...
	[[nodiscard]] static Digest digestFromHex(const std::string& digest);
		/// Converts a string created by digestToHex back to its Digest presentation

	[[nodiscard]] static bool constantTimeEquals(const Digest& d1, const Digest& d2);
