...
public:
  ...
  /// setting the target channel and log level, respectively, via the LoggingRegistry. The "channel" and "level" properties are set-only.
  ...
  void fatal(const std::string &fmt, T arg1, Args &&...args) {
    if (fatal())
      logAlways(Poco::format(fmt, arg1, std::forward<Args>(args)...),
                Message::PRIO_FATAL);
  }
  ...
  /// Replaces all occurrences of $<n> in fmt with the string given in arg<n> and returns the result. To include a dollar sign in the result string,
  /// specify two dollar signs ($$) in the format string.
  ...
  static void formatDump(std::string &message, const void *buffer,
                         std::size_t length);
  ...
  static void setProperty(const std::string &loggerName,
  ...
  static Logger &create(const std::string &name, Channel::Ptr pChannel,
...
protected:
  ...
  void log(const std::string &text, Message::Priority prio, const char *file,
           int line);

  static std::string format(const std::string &fmt, int argc,
                            std::string argv[]);
