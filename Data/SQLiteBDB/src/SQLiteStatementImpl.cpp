...
#include "Poco/Data/SQLiteBDB/SQLiteStatementImpl.h"
...
const int SQLiteBDBStatementImpl::POCO_SQLITE_INV_ROW_CNT = -1;


SQLiteBDBStatementImpl::SQLiteBDBStatementImpl(Poco::Data::SessionImpl& rSession, sqlite3* pDB):
...
SQLiteBDBStatementImpl::~SQLiteBDBStatementImpl()
{
	try
	{
		clear();
...
void SQLiteBDBStatementImpl::compileImpl()
{
	if (!_pLeftover)
	{
...
void SQLiteBDBStatementImpl::bindImpl()
{
	_stepCalled = false;
	_nextResponse = 0;
	if (_pStmt == 0) return;
...
void SQLiteBDBStatementImpl::clear()
{
...
bool SQLiteBDBStatementImpl::hasNext()
{
	if (_stepCalled)
...
std::size_t SQLiteBDBStatementImpl::next()
{
...
std::size_t SQLiteBDBStatementImpl::columnsReturned() const
{
	return (std::size_t) _columns[currentDataSet()].size();
}


const MetaColumn& SQLiteBDBStatementImpl::metaColumn(std::size_t pos) const
{
	std::size_t curDataSet = currentDataSet();
...
int SQLiteBDBStatementImpl::affectedRowCount() const
{
