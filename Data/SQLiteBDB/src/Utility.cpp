...
Utility::SQLiteBDBMutex::SQLiteBDBMutex(sqlite3* pDB): _pMutex((pDB) ? sqlite3_db_mutex(pDB) : 0)
{
	if (_pMutex)
...
Utility::SQLiteBDBMutex::~SQLiteBDBMutex()
{
	if (_pMutex)
...
void Utility::throwException(sqlite3* pDB, int rc, const std::string& addErrMsg)
{
	...
	default:
		throw SQLiteBDBException(Poco::format("Unknown error code: %d", rc), addErrMsg);
	}
}
